...
const MatchListItem = ({ username, match }) => {
  ...
  const user_info = players.filter((e) => e.summonerName === username)[0];
  const user_is_win = user_info.win;
  const user_item_arr = [
    user_info.item0,
    user_info.item1,
    user_info.item2,
    ...
    >
      <div className="Info">
        <div className="matchInfo">
          ...
          ></div>
          <div className="result">{user_info.win ? '승리' : '패배'}</div>
          ...
          <ul>
            {blue_team_players.map((player) => (
              <li key={'player' + player.id}>
                ...
                <div className="name">
                  <a
                    href={'summoners/' + player.summonerName}
                    style={{
                      fontWeight:
